...
import type { TimetableData, PeriodTime } from "./api.ts";
import { parseTime, timeToMinutes } from "./utils.ts";

// Default period times (empty until GAS data is loaded)
...
export function getSubjects(timetable: TimetableData | null): string[][] {
  // Returns subjects[periodIndex][dayIndex(0=Mon)]
  if (timetable?.subjects) return timetable.subjects;
  // Return empty 6x5 grid
...
export function getCurrentPeriodStatus(periods: PeriodTime[], now: Date): PeriodStatus {
  ...
  // Prep time (10 min before first class)
  ...
  for (let i = 0; i < periods.length; i++) {
    ...
    // Break between this period and next
    if (i + 1 < periods.length) {
      ...
      if (currentMinutes >= endMin && currentMinutes < nextStartMin) {
        ...
        // Check if this is lunch break (usually between period 4 and 5)
...
export function renderTimetable(
  ...
  periods: PeriodTime[],
  status: PeriodStatus,
  todayDayIndex: number // 0=Mon, 4=Fri, -1=weekend
): void {
  tableBody.innerHTML = "";
